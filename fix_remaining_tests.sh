...
echo "🔧 Исправление оставшихся тестов..."

# Обновляем simple_server.go для правильных ответов
cat > simple_server.go << 'NEWSERVER'
package main
...
func getPatients(w http.ResponseWriter, r *http.Request) {
    patients := []map[string]interface{}{
        ...
        {"full_name": "Иванов Иван", "birth_date": "1985-05-15"},
    }
...
<html><head><title>BOT_MAX</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:Arial;background:linear-gradient(135deg,#667eea,#764ba2);min-height:100vh}
.admin-btn{position:fixed;top:20px;right:20px;background:#667eea;color:white;padding:10px 20px;border-radius:30px;text-decoration:none}
.container{max-width:1200px;margin:0 auto;padding:40px}
.header{text-align:center;color:white}
.cards{display:grid;grid-template-columns:1fr 1fr;gap:30px;margin-top:40px}
.card{background:white;border-radius:20px;padding:30px;text-align:center}
.btn{background:linear-gradient(135deg,#667eea,#764ba2);color:white;padding:10px 20px;border-radius:30px;text-decoration:none}
...
<div class="cards">
<div class="card"><h2>👨‍⚕️ Пациент</h2><a href="/patient" class="btn">Войти</a></div>
<div class="card"><h2>👩‍⚕️ Врач</h2><a href="/doctor" class="btn">Войти</a></div>
</div>
</div>
</body></html>`)
}
...
func patientPage(w http.ResponseWriter, r *http.Request) {
    w.Header().Set("Content-Type", "text/html")
    fmt.Fprint(w, `<!DOCTYPE html>
<html><head><title>Пациент</title><meta name="viewport" content="width=device-width, initial-scale=1">
<style>
...
button{background:#667eea;color:white;border:none;padding:10px;border-radius:8px;cursor:pointer}
</style>
</head>
<body>
<div class="header"><h2>🏥 Личный кабинет пациента</h2><a href="/" style="color:white">Выйти</a></div>
<div class="container"><div class="card"><h3>📝 Ваши данные</h3><p>Добро пожаловать в систему!</p></div></div>
</body></html>`)
}
...
func doctorPage(w http.ResponseWriter, r *http.Request) {
    w.Header().Set("Content-Type", "text/html")
    fmt.Fprint(w, `<!DOCTYPE html>
<html><head><title>Врач</title><meta name="viewport" content="width=device-width, initial-scale=1">
<style>
body{font-family:Arial;background:#f0f2f5}
.header{background:linear-gradient(135deg,#667eea,#764ba2);color:white;padding:20px}
...
<div class="container"><div class="content"><h3>Кабинет врача</h3><p>Добро пожаловать!</p></div></div>
</body></html>`)
}
...
func adminPage(w http.ResponseWriter, r *http.Request) {
    w.Header().Set("Content-Type", "text/html")
    fmt.Fprint(w, `<!DOCTYPE html>
<html><head><title>Админ-панель</title><meta name="viewport" content="width=device-width, initial-scale=1">
<style>
...
.login-card{background:white;padding:40px;border-radius:20px;width:350px}
input{width:100%;padding:10px;margin:10px 0;border:1px solid #ddd;border-radius:8px}
button{width:100%;padding:10px;background:#667eea;color:white;border:none;border-radius:8px;cursor:pointer}
.dashboard{display:none}
.header{background:#16213e;padding:20px;color:white}
</style>
</head>
<body>
<div id="login" class="login-container"><div class="login-card"><h2>🔐 Админ-панель</h2><input type="text" id="user" placeholder="Логин"><input type="password" id="pass" placeholder="Пароль"><button onclick="login()">Войти</button></div></div>
<div id="dashboard" class="dashboard"><div class="header"><h2>🏥 Админ-панель</h2><button onclick="logout()">Выйти</button></div></div>
<script>
function login(){var u=document.getElementById('user').value,p=document.getElementById('pass').value;if(u=='admin'&&p=='admin123'){document.getElementById('login').style.display='none';document.getElementById('dashboard').style.display='block'}else alert('Ошибка')}
...
nohup go run simple_server.go > server.log 2>&1 &
sleep 3

echo ""
echo "✅ Сервер обновлен и перезапущен!"
echo ""
echo "Запустите тестирование снова:"
